...
First of all, the dataset used consists of Rivalry data, Alliance data, and State Dependency data. By filtering the edges of type Rivalry and Alliance from the original data, we could generate the Rivalry Network and Alliance Network respectively. All isolates were removed from the Rivalry Network in order to only retain the cartels involved in competitive relationships.

The dataset is structured such that each row represents a unique combination of a cartel and a state in which it operates. Therefore, after the data was cleaned the data was aggregated by cartel name. This ensured that every cartel has an unique node, with their alliances and rivalries, as well as operational attributes, consolidated in a manner representative of reality. The degree and centrality of each node in the Alliance Network were calculated and added as node attributes to the vertices in the Rivalry Network. The number of states in which the cartels are active were counted and added as an attribute of the node (as another proxy for geographic extensiveness of operation of the cartel).

In order to investigate the shared geographical locations between cartels two state dependency matrices were created. One binary matrix, "state_dependency_matrix1", and one weighted, "state_dependency_matrix2". The "state_dependency_matrix1" provides for each pair of cartels if they are present in the same state; while "state_dependency_matrix2" accounts for the number of shared states between the pairs. Lastly, the network was transformed into an object that could be used by ERGM for analysis to fit into the ERGM framework. NA values were checked in the final network, but no missing values have been identified.
...
### Degree Distirubution of Rivalry Network
...
```{r, warning = FALSE, fig.width=0.5, fig.height=1}
texreg::screenreg(
  list(m1, m2, m3),
