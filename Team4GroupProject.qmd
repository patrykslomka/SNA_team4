...
#| echo: false
cat("Table1: The Final Model Results\n")
texreg::screenreg(m9,custom.model.names = "Model 9")
...
required_packages <- c("readxl", "ggplot2", "igraph", "ggcorrplot")
...
# Create the bar plot
ggplot2::ggplot(state_counts_df, ggplot2::aes(x = State, y = Count)) +
  ggplot2::geom_bar(stat = "identity", fill = "steelblue") +
  ggplot2::labs(title = "Figure1:Number of Cartels Active in Each State", x = "State", y = "Count") +
...
rivalry_graph <- igraph::graph_from_data_frame(rivalry_edges, directed = FALSE)

# Plot degree distribution
degree_distribution <- igraph::degree(rivalry_graph)
ggplot2::ggplot(data.frame(Degree = degree_distribution), ggplot2::aes(x = Degree)) +
  ggplot2::geom_histogram(bins = 20, fill = "darkorange", color = "black") +
  ggplot2::labs(title = "Figure2: Degree Distribution of Rivalry Network", x = "Degree", y = "Frequency")
```
...
ggcorrplot::ggcorrplot(activity_correlation, 
                       method = "square", 
                       lab = TRUE, 
                       title = "Figure2: Correlation Matrix of Cartel Activities")

```

## Appendix B: ERGM Modeling

```{r, fig.width=8, fig.height=6}

# Generate GOF statistics
gofm <- ergm::gof(m9)

# Plot GOF using snafun
snafun::stat_plot_gof(gofm)
```
\pagebreak
...
```{r}
#| echo: false
cat("Table 2: ERGM Model Results for Models 1-3")
```
...
```{r}
#| echo: false
cat("Table 3: ERGM Model Results for Models 4-6")
```
...
```{r}
#| echo: false
cat("Table 4: ERGM Model Results for Models 7-9")
```

## Appendix C: ERGM Terms

| Term                             | RQ       | Hypothesis       | Role                                                                 | Interpretation                                                                                                                                                                                                 |
|----------------------------------|----------|------------------|----------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| edges                            | All      | Baseline         | Captures the baseline likelihood of rivalry ties forming             | Negative coefficient indicates rivalry ties are less likely to form without other structural or covariate effects.                                                                                           |
| gwdsp.fixed.0.5                  | RQ1      | Hypothesis 1.1   | Captures the influence of indirect ties (paths of length 2)          | Positive coefficient indicates that indirect connections (e.g., common enemies) reduce rivalry formation.                                                                                                   |
| nodecov.alliance_degree          | RQ2      | Hypothesis 2.2   | Measures the direct connections of a cartel in the alliance network  | Positive coefficient suggests cartels with more alliances tend to have more rivals.                                                                                                                          |
| nodecov.alliance_eigenvector     | RQ2      | Hypothesis 2.1   | Tests whether broader influence in the alliance network impacts rivalry formation | Not significant, indicating broader influence is less impactful than direct connections.                                                                               |
| kstar2                           | RQ2      | Hypothesis 2.3   | Measures structural aggressiveness through shared ties               | Negative coefficient shows cartels avoid multi-party conflicts (e.g., multi-party conflicts), preferring hierarchical structures.                                                                             |
| nodecov.num_states               | RQ3      | Hypothesis 3.1   | Measures geographic reach by counting the states a cartel is active in | Not significant, suggesting that cross-state activity does not strongly predict rivalry formation.                                                                                                           |
| edgecov.state_dependency_matrix2 | RQ3      | Hypothesis 3.2   | Captures the effect of geographic proximity on rivalry formation     | Positive coefficient indicates cartels in the same state are more likely to become rivals.                                                                                                                   |
| degree1                          | Control  | Control          | Control variable to account for peripheral cartels                   | Positive coefficient suggests low-degree nodes still influence rivalry formation.                                                                                                                            |
| gwesp(0.5)                       | Control  | Control          | Clustering                                                           | Not significant; clustering does not play a major role in rivalry formation.                                                                                                                                 |