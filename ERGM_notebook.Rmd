...
node_names_with_na <- get.vertex.attribute(g_igraph, "vertex.names")[na_indices]
node_names_with_na
...
m4 <- ergm::ergm(
  g ~ edges + 
    ...
    edgecov(state_dependency_matrix1) +
    kstar(2) +
    nodecov("num_states"), #the state dependency matrix is still significant, so we can remove this one
  control = ergm::control.ergm(
    MCMC.burnin = 5000,
    MCMC.samplesize = 10000,
    seed = 123456,
...
m5 <- ergm::ergm(
  g ~ edges + 
    ...
    kstar(2) + 
    nodecov("num_states") +
    degree(1),#degree improves the model
  control = ergm::control.ergm(
    MCMC.burnin = 5000,
    MCMC.samplesize = 10000,
    seed = 123456,
...
m6 <- ergm::ergm(
  g ~ edges + 
    ...
    degree(1) +
    nodecov("num_states") +
    gwdsp(0.5, fixed = TRUE), #gwdsp doesnt improve the model
  control = ergm::control.ergm(
    MCMC.burnin = 5000,
    MCMC.samplesize = 10000,
    seed = 123456,
...
m7 <- ergm::ergm(
  g ~ edges + 
    gwesp(0.5, fixed = TRUE) + 
    nodecov("alliance_eigenvector") + 
    edgecov(state_dependency_matrix2) +
    kstar(2) +
    degree(1) +
    nodecov("num_states") +
    gwdsp(0.5, fixed = TRUE), #gwdsp doesnt improve the model
...
m8 <- ergm::ergm(
  g ~ edges + 
    ...
    kstar(2) +
    degree(1) +
    nodecov("num_states"), #gwdsp doesnt improve the model
  control = ergm::control.ergm(
    MCMC.burnin = 5000,
    MCMC.samplesize = 10000,
    seed = 123456,
...
```
```{r, fig.width=8, fig.height=6} 
# Normalize the matrix to create a proportional shared state matrix
normalized_matrix <- state_dependency_matrix2 / max(state_dependency_matrix2)
