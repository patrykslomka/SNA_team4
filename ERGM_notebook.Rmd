...
```{r}
# Combine networks into a list for BTERGM
# Ensure the graph is undirected
g <- igraph::as.undirected(g, mode = "collapse")
network_list <- list(g)

# Define the model formula (using degree term correctly with btergm::edges and btergm::degree)
model_formula <- network_list ~ edges + degree(1)

# Fit the BTERGM model with R bootstrap replicates
btergm_model <- btergm::btergm(
  model_formula,
  R = 1000  # Set the number of bootstraps as needed
)

fit.gof <- btergm::gof(fit)
snafun::stat_plot_gof(fit.gof)

# Display model summary using btergm::summary.btergm
print(btergm::summary.btergm(btergm_model))
...
alliance_graph <- subgraph.edges(g, E(g)[E(g)$type == "alliance"])
